        ...
        <div className="space-y-2">
          ...
          {categories.map(cat => (
            <button
              ...
              className={`block w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${selectedCategory === cat.slug ? 'bg-primary/10 text-primary font-medium' : 'text-muted-foreground hover:text-foreground hover:bg-secondary'}`}
            >
              {cat.name}
            </button>
          ))}
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <h1 className="text-3xl md:text-4xl font-bold mb-2">
            {selectedCategory !== 'all' ? categories.find(c => c.slug === selectedCategory)?.name || 'Products' : 'All Products'}
          </h1>
          ...
          <div className="flex-1">
            {loading ? (
              ...
              <div className="text-center py-20">
                ...
                <Button variant="outline" onClick={() => { setSelectedCategory('all'); setSearchQuery(''); setPriceRange([0, maxPrice]); }}>
                  Clear Filters
                </Button>
              </div>
            ) : (
